class ReportsController < ApplicationController
  ...
  def default
    select = 'DATE(created_at) as row_date, SUM(stats.ios) as ios, SUM(stats.android) as android, SUM(stats.other_os) as other_os, SUM(facebook) as facebook, SUM(googleplus) as googleplus, SUM(other_sn) as other_sn,  SUM(stats.views) as views'
    ...
    @data = {}
    @views.each do |view|
      @data[view[0]] = {:date => view[0], :ios => view[1].ios, :android => view[1].android, :other_os => view[1].other_os, :facebook => view[1].facebook, :googleplus => view[1].googleplus, :other_sn => view[1].other_sn, :views => view[1].views, :shares => 0}
    end
